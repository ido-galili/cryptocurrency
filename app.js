...
const miliToSecDivisor = 60000 // miliseconds
const minutesToFetchInfoAgain = 2

const coinsLimit = 20 // Dev related

let allCoinsApiURL = 'https://api.coingecko.com/api/v3/coins/list'
let oneCoinApiURL = 'https://api.coingecko.com/api/v3/coins/' // + {id}
...
class Coin {
    ...
    isTimeExpired() {
        ...
        let elapsed = (now - this.created) / miliToSecDivisor // elapsed time in milliseconds converted to seconds

        if (elapsed < minutesToFetchInfoAgain) {
            return false
        }
    ...
    $('.card-info-link').click(event => {
        ...
        if (window.localStorage.moreInfoMap) {
            ...
            if (moreInfoMap.has(id)) {
                ...
                // Check if time of last fetch exceeds minutesToFetchInfoAgain
...
function getMoreInfo(id) {
    let url = `${oneCoinApiURL}${id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`
    var newCoin = {}
    ...
    $.ajax({
        ...
        success: (coin) => {
            ...
            newCoin = new Coin(
                ...
                coin.name,
                coin.image.small, {
                    usd: coin.market_data.current_price.usd.toFixed(digitsAfterDecimal),
                    eur: coin.market_data.current_price.eur.toFixed(digitsAfterDecimal),
                    ils: coin.market_data.current_price.ils.toFixed(digitsAfterDecimal)
                })
        },
        error: (error) => {
            console.log("error : ", error)
